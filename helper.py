...
def clean_spark_temperature_data(df):
    ...
    print('Number of rows after removing misssing values.: {:,}'.format(total_records-total_recs_after_dropping_nas))
    
    # drop duplicate rows
    df = df.drop_duplicates(subset=['dt', 'City', 'Country'])
    print('Number of rows removed after finding duplicate values: {:,}'.format(total_recs_after_dropping_nas-df.count()))
...
def clean_spark_demographics_data(df):
    ...
    print("Number of rows removed due to missing values: {}".format(rows_dropped))
    
    # drop duplicate columns
    new_df2 = new_df.dropDuplicates(subset=['City', 'State', 'State Code', 'Race'])
    
    rows_dropped_with_duplicates = new_df.count()-new_df2.count()
    print(f"Number of rows removed after duplicates is found: {rows_dropped_with_duplicates}")
