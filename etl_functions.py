...
from helper import aggregate_temperature_data
...
#perform uniqueness quality check
def quality_check_unique(df,table_name,primary_key):
    """
        Uniqueness quality check verifies the uniquness of the data in each table
        
        input:
        df : spark dataframe 
        table_name: corresponding name of the table
        primary_key: unique key
    """
    unique_count = df.select(countDistinct(primary_key)).show()
    total_count = df.count()
